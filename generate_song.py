"""Full YuE2 MLX song generation - memory-optimized VAE decode"""
import gc
import time
import sys
...
print("="*60)
print("YuE2 MLX - Song Generation (Memory Optimized)")
print("="*60)
...
def sample_logits(logits, temperature=0.7, top_k=50, top_p=0.95, repetition_penalty=1.2, history=None):
    """Sample from logits. logits: 1D array"""
    if logits.ndim > 1:
        logits = logits.reshape(-1)
    
    if history and repetition_penalty != 1.0:
        recent = history[-50:] if len(history) > 50 else history
        penalty_arr = mx.ones_like(logits)
        for token_id in recent:
            if 0 <= token_id < len(logits):
                if logits[token_id] < 0:
                    penalty_arr[token_id] = repetition_penalty
                else:
                    penalty_arr[token_id] = 1.0 / repetition_penalty
        logits = logits * penalty_arr
    ...
    if top_k > 0:
        ...
        logits = mx.where(logits >= top_vals[0], logits, -1e9)
    
    if top_p < 1.0:
        probs = mx.softmax(logits)
        sorted_probs = mx.sort(probs)[::-1]
        cumsum = mx.cumsum(sorted_probs)
        n_keep = int(mx.sum(cumsum < top_p).item()) + 1
        threshold = sorted_probs[min(n_keep, len(sorted_probs)-1)]
        probs = mx.where(probs >= threshold, probs, 0.0)
        probs = probs / mx.sum(probs)
        return int(mx.random.categorical(probs[None, :], num_samples=1).item())
...
for i in range(100):
    ...
    logits_abc = logits[0, -1, :]
    mask = (mx.arange(len(logits_abc)) < EOD) | (mx.arange(len(logits_abc)) == ABC_END)
    logits_abc = mx.where(mask, logits_abc, -1e9)
    
    next_token = sample_logits(logits_abc, temperature=0.7, top_k=30, top_p=0.9,
...
for i in range(200):
    ...
    logits_codec = logits[0, -1, CODEC_OFFSET:CODEC_OFFSET+CODEC_SIZE]
    
    next_idx = sample_logits(logits_codec, temperature=1.0, top_k=100, top_p=0.95,
                              repetition_penalty=1.2, history=codec_ids)
    
    next_token = CODEC_OFFSET + next_idx
...
print(f"  Codec: {len(codec_ids)} tokens in {codec_time:.2f}s")
...
max_ar_context = min(50, len(codec_values))
codec_context = codec_values[:max_ar_context]

mx.random.seed(42)
num_latent_frames = len(codec_context) * 10
...
# Free LM memory before VAE decode
del model
gc.collect()

# ============ VAE DECODE (memory-efficient tiled) ============
print("\n[Step 4] Decoding to audio (tiled for memory)...")

latents_np = np.array(latents.T)  # [64, T]
total_frames = latents_np.shape[1]
core_frames = 512
halo_frames = 16
audio_chunks = []

for start in range(0, total_frames, core_frames):
    end = min(total_frames, start + core_frames)
    # Add halo context
    left = max(0, start - halo_frames)
    right = min(total_frames, end + halo_frames)
    tile = latents_np[:, left:right][None, :, :]  # [1, 64, T_tile]
    
    tile_tensor = mx.array(tile)
    audio_tile = decoder(tile_tensor)
    mx.eval(audio_tile)
    audio_np = np.array(audio_tile)[0]  # [2, T_out_tile]
    
    # Crop halo from output
    crop_left = (start - left) * 1920
    crop_end = crop_left + (end - start) * 1920
    audio_chunks.append(audio_np[:, crop_left:crop_end])
    
    del tile_tensor, audio_tile, audio_np
    gc.collect()
...
output = "/tmp/yue2-mlx/yueml_song_v3.wav"
...
print(f"{'='*60}")
print(f"Duration: {duration:.2f}s | 48000Hz | Channels: {audio.shape[0]}")
