"""Full YuE2 MLX song generation - v3"""
import gc
import time
import sys
...
print("="*60)
print("YuE2 MLX - Full Pipeline v3")
print("="*60)
...
for i in range(100):
    ...
    # Slice to valid ABC range: 0..EOD-1 + ABC_END
    logits_abc = mx.concatenate([
        logits[:, :, :EOD],
        logits[:, :, ABC_END:ABC_END+1]
    ], axis=-1)
    
    next_idx = sample_logits(logits_abc[:, -1, :], temperature=0.7, top_k=30, top_p=0.9,
...
for i in range(200):
    ...
    # Slice to valid codec range + MUSIC_END
    logits_codec = mx.concatenate([
        logits[:, :, CODEC_OFFSET:CODEC_OFFSET+CODEC_SIZE],
        logits[:, :, MUSIC_END:MUSIC_END+1]
    ], axis=-1)
    ...
    # Map back to original token ID
    if next_idx < CODEC_SIZE:
        next_token = CODEC_OFFSET + next_idx
    else:
        next_token = MUSIC_END
...
output = "/tmp/yue2-mlx/yueml_full_pipeline_v3.wav"
